...
void MerkleTree_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MerkleTree_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 12400;}
...
Fr_lt(&expaux[0],&lvar[16],&circuitConstants[17]); // line circom 10
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[((1 * Fr_toInt(&lvar[16])) + 1)];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[16])) + 1)]);
}
{
PFrElement aux_dest = &lvar[16];
// load src
Fr_add(&expaux[0],&lvar[16],&circuitConstants[3]); // line circom 10
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[16],&circuitConstants[17]); // line circom 10
...
Fr_lt(&expaux[0],&lvar[16],&circuitConstants[15]); // line circom 14
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[16])) + 0);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 3];
// load src
// end load src
...
// load src
// end load src
Fr_copy(aux_dest,&lvar[((1 * (2 * Fr_toInt(&lvar[16]))) + 1)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
MiMCSponge_1_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&lvar[((1 * ((2 * Fr_toInt(&lvar[16])) + 1)) + 1)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
MiMCSponge_1_run(mySubcomponents[cmp_index_ref],ctx);
...
{
PFrElement aux_dest = &lvar[((1 * (Fr_toInt(&lvar[16]) + 8)) + 1)];
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[16])) + 0)]].signalStart + 0]);
}
{
PFrElement aux_dest = &lvar[16];
// load src
Fr_add(&expaux[0],&lvar[16],&circuitConstants[3]); // line circom 14
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[16],&circuitConstants[15]); // line circom 14
...
MerkleTree_2_create(1,0,ctx,"main",0);
MerkleTree_2_run(0,ctx);
}
